...
import MultiDayToggle from "@/components/MultiDayToggle";
import BirthDetailsForm from "@/components/BirthDetailsForm";
import AreaSelector from "@/components/AreaSelector";
import MaterialsArrangement from "@/components/MaterialsArrangement";
import { Service, Priest, formatINR, formatDuration } from "@/lib/types";
...
const BookService = () => {
  ...
  const { data: serviceRows } = useQuery<Service[]>({
  ...
  const handleConfirm = async () => {
    if (!user) {
      ...
      navigate("/signin?next=/book/" + slug);
      return;
    }
    ...
    const reference = "SBK-" + Math.random().toString(36).slice(2, 8).toUpperCase();

    await db.insert("bookings", {
      reference,
      service_slug: service!.slug,
      service_name: service!.name,
      priest_slug: priest?.slug || null,
      ...
      payment_status: "paid",
      status: "confirmed",
      is_multi_day: multiDay ? 1 : 0,
      end_date: multiDay && endDate ? endDate : null,
      birth_details: birthDetails || null,
      partner_birth_details: partnerDetails || null,
      materials_arrangement: materialsArrangement,
    });

    setSubmitting(false);
    toast.success("Booking confirmed — reference " + reference);
    navigate("/bookings");
  };
  ...
  return (
    <Layout>
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        >
          <ArrowLeft className="h-4 w-4" /> Back to {service.name}
        </Link>

        <h1 className="mt-4 text-3xl text-foreground">Book your pooja</h1>

        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_340px]">
          <div className="space-y-8">
            <section className="rounded-2xl border border-border bg-card p-6">
              <h2 className="font-display text-lg text-foreground">1. Choose your acharya</h2>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    We'll assign the best match for your tradition.
                  </p>
                </button>
                {priests.map((p) => (
                  <button
            ...
            <section className="rounded-2xl border border-border bg-card p-6">
              <div className="flex items-center justify-between">
                <h2 className="font-display text-lg text-foreground">2. Pick date & time</h2>
                <span className="text-xs text-muted-foreground">⏰ 3-hour advance booking (evening slots always available)</span>
              </div>
              ...
              {slot && (
                <p className="mt-4 text-sm text-muted-foreground bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg px-3 py-2">
                  ⏱️ Your booking is for <span className="font-medium text-foreground">{slot}</span> on <span className="font-medium text-foreground">{new Date(date).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</span>. 
                  Evening slots (7 PM onwards) are always available for same-day booking. Other slots require 3-hour notice for priest travel.
                </p>
              )}
              {!slot && date === todayISO() && (
              ...
              {service.category === "Astrology" && (
                <div className="mt-6">
                  <BirthDetailsForm
                    ...
                    showPartner={service.slug === "kundali-matching"}
                  />
                </div>
              )}
            </section>
            ...
            <section className="rounded-2xl border border-border bg-card p-6">
              <h2 className="font-display text-lg text-foreground">3. Pooja materials</h2>
              <div className="mt-4">
            ...
            <section className="rounded-2xl border border-border bg-card p-6">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <Label htmlFor="notes">Gotra, nakshatra or special requests</Label>
                  <Textarea
                    id="notes"
                    className="mt-1.5"
                    rows={3}
